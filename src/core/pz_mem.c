...
static void *
pz_alloc_internal(size_t size, pz_mem_category category)
{
    if (size == 0)
        return NULL;

    // Allocate header + user data
    pz_alloc_header *header
        = (pz_alloc_header *)malloc(sizeof(pz_alloc_header) + size);
    if (!header)
        return NULL;
...
void *
pz_alloc(size_t size)
{
    return pz_alloc_internal(size, PZ_MEM_GENERAL);
}

void *
pz_alloc_tagged(size_t size, pz_mem_category category)
{
    return pz_alloc_internal(size, category);
}

void *
pz_calloc(size_t count, size_t size)
{
    return pz_calloc_tagged(count, size, PZ_MEM_GENERAL);
}

void *
pz_calloc_tagged(size_t count, size_t size, pz_mem_category category)
{
...
void *
pz_realloc(void *ptr, size_t new_size)
{
    if (!ptr) {
    ...
    // Realloc
    pz_alloc_header *new_header = (pz_alloc_header *)realloc(
        header, sizeof(pz_alloc_header) + new_size);
    if (!new_header) {
...
size_t
pz_mem_get_allocated(void)
{
    return g_total_allocated;
}

size_t
pz_mem_get_alloc_count(void)
{
    return g_alloc_count;
}

size_t
pz_mem_get_category_allocated(pz_mem_category category)
{
    if (category >= PZ_MEM_CATEGORY_COUNT)
        return 0;
    return g_category_allocated[category];
}

void
pz_mem_dump_leaks(void)
{
    if (!g_alloc_list) {
