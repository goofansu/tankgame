...
#define ASSERT_EQ(expected, actual) do { \
    if ((expected) != (actual)) { \
        printf(PZ_COLOR_RED "  FAIL: %s:%d: expected %d, got %d" PZ_COLOR_RESET "\n", \
...
#define ASSERT_STR_EQ(expected, actual) do { \
    if (strcmp((expected), (actual)) != 0) { \
        printf(PZ_COLOR_RED "  FAIL: %s:%d: expected \"%s\", got \"%s\"" PZ_COLOR_RESET "\n", \
...
#define ASSERT_NEAR(expected, actual, epsilon) do { \
    ...
    if (_diff > (epsilon)) { \
        printf(PZ_COLOR_RED "  FAIL: %s:%d: expected %.6f, got %.6f (diff=%.6f)" PZ_COLOR_RESET "\n", \
...
#define ASSERT_NOT_NULL(ptr) do { \
    if ((ptr) == NULL) { \
        printf(PZ_COLOR_RED "  FAIL: %s:%d: unexpected NULL" PZ_COLOR_RESET "\n", \
               __FILE__, __LINE__); \
        pz_tests_failed++; \
        return; \
    } \
...
// Run all tests
static int pz_run_all_tests(void) {
    ...
    if (pz_tests_passed > 0) printf(PZ_COLOR_GREEN "%d passed" PZ_COLOR_RESET ", ", pz_tests_passed);
    if (pz_tests_failed > 0) printf(PZ_COLOR_RED "%d failed" PZ_COLOR_RESET, pz_tests_failed);
